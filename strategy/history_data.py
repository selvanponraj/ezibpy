...
if __name__ == '__main__':
    ...
    # import pandas as pd
    # bars = pd.read_csv(dirpath + 'AAL' + '.csv')
    # bars = bars.set_index(pd.DatetimeIndex(bars['datetime']))
    # bar_time_format ='%Y-%m-%d %H:%M:%S.%f'
    # bar_start_time = algo_time.replace(hour=14).replace(minute=15).replace(second=00).strftime(bar_time_format)
    # bar_end_time = algo_time.replace(hour=17).replace(minute=15).replace(second=00).strftime(bar_time_format)
    # print(bars.index)
